...
int main(int argc, char **argv) {
	...
	if (process_arguments(argc, argv, &buffer_size, &num_messages,
	...
	pthread_t c_thread_id[num_consumers];

	//creates structs to pass to threads
	struct thread_params p_id[num_producers];
	struct thread_params c_id[num_consumers];
...
void* producer(void* producer_params) {
	...
	int i;
	for (i = p_id; i < num_messages; i += num_producers) {
